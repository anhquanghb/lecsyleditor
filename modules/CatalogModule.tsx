...
const CatalogModule: React.FC<Props> = ({ state, updateState }) => {
  // Destructured knowledgeAreas and departments from state
  const { courses, language, knowledgeAreas, generalInfo, departments } = state;
  ...
  const updateCourseDepartment = (id: string, deptId: string) => {
    updateState(prev => ({
      ...prev,
      courses: prev.courses.map(c => c.id === id ? { ...c, departmentId: deptId } : c)
    }));
  };
